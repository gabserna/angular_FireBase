...
export class CompanyEditComponent implements OnInit {
  ...
  saveCompany(company: Company) {
    this.id ? company.id = this.id : "";
    this.companyService.saveCompany(company)
      .subscribe(() => this.router.navigate(["company/all"]));
  }

  editCompany(company: Company) {
    this.id ? company.id = this.id : "";
    console.log("Company: " + JSON.stringify(company))
    this.companyService.editCompany(company).then(
      () => this.router.navigate(["company/all"])
    );
  }

  deleteCompany() {
    if(this.id) this.companyService.deleteCompany(this.id).then(
      () => this.router.navigate(["company/all"])
    );
  }
